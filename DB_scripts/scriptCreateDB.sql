...
show tables;
select vtaFecha from venta;
select * from libro where libId=1003;
select libId, count(vtaId) as "Contas Ventas" from venta group by (libId);
select autId, avg (libPrecio) as "Promedio" from libro group by (autId);
select libId, count(vtaId) from venta group by (libId) having count(vtaId) > 2 limit 3;
select LibNombre,libPrecio from libro order by libPrecio desc limit 5;